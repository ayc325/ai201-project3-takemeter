...
Original file is located at
    https://colab.research.google.com/drive/15Pt67TX8i4IllqFRtXOTIUFR8QZPtplC
...
LABEL_MAP = {
    "news":  0,   # ← Replace with your first label
    "discussion":  1,   # ← Replace with your second label
    "appreciation":  2,   # ← Replace with your third label (remove if you have 2 labels)
    "comparison": 3,  # ← Uncomment if you have a fourth label
}
...
def tokenize(examples):
    return tokenizer(examples["Reddit Post"], truncation=True, max_length=256)

def make_dataset(df_split):
    ds = Dataset.from_pandas(
        df_split[["Reddit Post", "label_id"]].rename(columns={"label_id": "labels"})
    )
...
for i, idx in enumerate(wrong_idx[:15]):
    text = test_df.iloc[idx]["Reddit Post"]
...
for i, (_, row) in enumerate(test_df.iterrows()):
    pred = classify_with_groq(row["Reddit Post"])
